...
namespace InventoryOrder.Controllers
{
    public class ProductsController : Controller
    {
        ...
        public ProductsController(IRepository<Product> _Productrepository, IRepository<OrderDetail> _OrderDetailRepository,
            IRepository<Warehouse> _Warehouserepository , IRepository<PurchaseDetail> _PurchaseDetailRepository
            )
        {
           
            _productRepository = _Productrepository;
            _orderDetailRepository = _OrderDetailRepository;
        ...
        //    var totalPurchased = _purchaseDetailRepository.GetAll()
        //         .Where(p => p.ProductID == id)
        //         .Sum(od => od.Quantity);

        //    var totalSold = _orderDetailRepository.GetAll()
        //         .Where(p => p.ProductID == id)
        //         .Sum(od => od.Quantity);
        ...
        //    var topCustomer = _orderDetailRepository.GetAll()
        //        .Where(p => p.ProductID == id)
        ...
        public IActionResult Details(int id)
        {
            ...
            // Find the customer who bought the most of this product
            var topCustomer = _orderDetailRepository.GetAll()
                .Where(od => od.ProductID == id && od.Order != null && od.Order.Customer != null)
                .GroupBy(od => od.Order.Customer)
                .OrderByDescending(g => g.Sum(od => od.Quantity))
                .FirstOrDefault()?.Key;
