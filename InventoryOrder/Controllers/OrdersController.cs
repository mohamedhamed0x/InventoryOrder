...
namespace InventoryOrder.Controllers
{
    public class OrdersController : Controller
    {
        ...
        public IActionResult Details(int id)
        {
            ...
            var orderDetails = _orderDetailRepository.GetAll("Product").Where(od => od.OrderID == id).ToList();
            ...
            if (order != null && orderDetails != null)
            {
                using (var transaction = new TransactionScope())
                {
                    try
                    {
                        ...
                        // هنا نتحقق من المخزون قبل إضافة تفاصيل الطلب
                        foreach (var detail in orderDetails)
                        {
                            if (!_inventoryService.DecreaseStock(detail.ProductID, detail.Quantity))
                            {

                                ModelState.AddModelError("", $"Insufficient stock for product: {detail.Product.Name}");
        ...
        {
            var availableStock = _inventoryService.GetAvailableStock(productId);
            bool isAvailable = availableStock >= quantity;
            return Json(new { isAvailable, availableStock });
        }
